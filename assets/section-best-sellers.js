function initBestSellersSplit() {
 ...
 const sections = document.querySelectorAll("[data-split-section]");
  ...
  sections.forEach((section) => {
    ...
    // Хендлер для Shopify Theme Editor (Кастомайзера)
    // Коли клікають на блок в адмінці — фронтенд автоматично перемикає таб
    ...
    section.addEventListener('click', (e) => {
        ...
        // 2. КЛІК НА ОКО (Quick Add to Cart via Ajax API)
        ...
        if (quickAddBtn) {
          ...
          .then(data => {
            ...
            setTimeout(() => quickAddBtn.style.backgroundColor = '#ffffff', 1000);
          })
          .catch(error => {
